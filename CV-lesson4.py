...
img = cv2.imread('photo/photo5.png')
scale = 1
...
#Тримає тільки непарні числа
img_copy = cv2.equalizeHist(img_copy)
#Посиленням контрасту
img_copy = cv2.Canny(img_copy, 75, 120)
#Пошук контурів
contours, hierarchy = cv2.findContours(img_copy, cv2.RETR_EXTERNAL, cv2.CHAIN_APPROX_SIMPLE)
#RETR_EXTERNAL - режим отпримання контурів, знаходить крайній контур, якщо об'єкт має дірку, то вона буде ігноруватися
#Апроксимаціця - процес наближеного вираження одних величин через інші
...
#boundingRect створюєє найменший прямокутник, який містить у собі контур


cv2.imshow("Edges and coordinates", img)
cv2.imshow("Copy border", img_copy)
cv2.imshow("Canny edges", img_copy_color)
cv2.waitKey(0)
